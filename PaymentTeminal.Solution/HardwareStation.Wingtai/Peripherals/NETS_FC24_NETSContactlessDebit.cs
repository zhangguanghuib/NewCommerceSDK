...
{
    public class NETS_FC24_NETSContactlessDebit : NETS
    {
        ...
        protected override StringBuilder constructECRRequest(string _amount, string _ECR)
        {
            try
            {
                StringBuilder ECRFormat = new StringBuilder("0000000000");
        ...
        protected override byte[] constructRequestMessageToTerminal(string _amount, string _ECR)
        {
            try
            {
                ...
                byte[] MessageHeader = GetMessageHeader("24");
                byte[] MessageDataTransaction = GenerateMessageData("40", 12, _amount);

                // WTR RAD NETS CERT
                //byte[] MessageDataECR = GenerateMessageData("H4", 13, _ECR);
                //if (_ECR.Length >= 10)
                //{
                //    _ECR =  _ECR.Substring(3, 10);
                //}

                byte[] MessageDataECR = GenerateMessageData("H4", 10, _ECR);

                byte[] buffer = new byte[MessageHeader.Length + MessageDataTransaction.Length + MessageDataECR.Length + 3];
