...
export default class ListInputDialog {
    public show(context: IExtensionContext, message: string, array: ListData[]): Promise<string> {
        let promise: Promise<string> = new Promise<string>((resolve: (num: string) => void, reject: (reason?: any) => void) => {
            ...
            let listItems: ListData[] = array;//[new ListData("Houston", "1"), new ListData("Seattle", "2"), new ListData("Boston", "3")];
            ...
            let listInputDialogOptions: IListInputDialogOptions = {
                title: context.resources.getString("string_14"), // string that denotes the optional title on the list dialog
                subTitle: context.resources.getString("string_14"), // string that denotes the optional subtitle under the title
                items: convertedListItems//,
                //onBeforeClose: this.onBeforeClose.bind(this, array)
            };
            ...
            context.runtime.executeAsync(dialogRequest)
                .then((result: ClientEntities.ICancelableDataResult<ShowListInputDialogClientResponse>) => {
                    if (!result.canceled) {
                        ...
                        resolve(selectedListData.value);
                    } else {
    ...
    // */
    //private onBeforeClose(result: ClientEntities.ICancelableDataResult<IListInputDialogResult>, array: ListData[]): Promise<void> {
    //    if (!result.canceled) {
    //            if (result.data.value.label === "Houston") {

    //                /* new List data that you may want to display after displaying error message */
    //                let listItems: ListData[] = array;// [new ListData("Pittsburgh", "1"), new ListData("Seattle", "2"), new ListData("Boston", "3")];
    //                let convertedListItems: IListInputDialogItem[] = listItems.map((listItem: ListData): IListInputDialogItem => {
    //                    let convertedListItem: IListInputDialogItem = {
    //                        label: listItem.label, // string to be displayed for the given item
    //                        value: listItem // list data item that the string label represents
