...
export default class CreditCardOperationRequestHandler<TResponse extends PaymentOperationResponse> extends ExtensionOperationRequestHandlerBase<TResponse> {


    public supportedRequestType(): ExtensionOperationRequestType<TResponse> {
        return CreditCardOperationRequest;
    }


    public executeAsync(request: CreditCardOperationRequest<TResponse>): Promise<ClientEntities.ICancelableDataResult<any>> {
        this.context.logger.logInformational("CreditCardOperationRequestHandler executeAsync().");
                        ...
                        return this.context.runtime.executeAsync(getDeviceConfigurationRequest).then((result1) => {
                            ...
                            return this.context.runtime.executeAsync(tenderTypeRequest).then((result1: ClientEntities.ICancelableDataResult<GetOrgUnitTenderTypesClientResponse>) => {
                                result1.data.result.forEach((response) => {
                                    if (response.TenderTypeId == CreditCardOperationRequest.PAYMENT_METHOD) {
                                        tenderType(response);
                                    }
                                });
                                let options: IPaymentExtensionViewModelOptions = {
                                    tenderId: CreditCardOperationRequest.PAYMENT_METHOD,
                                    cart: cart(),
