...
export default class ExampleViewModel {
    ...
    public runPingTest(): Promise<void> {
        return this._context.runtime
            .executeAsync(new Messages.StoreOperations.ValidateCustomerDetailsRequest('GH80@Z80.com'))
            .then((validateCustomerDetailsResponse: ClientEntities.ICancelableDataResult<Messages.StoreOperations.ValidateCustomerDetailsResponse>): Promise<void> => {
                let pingResultDialog: PingResultDialog = new PingResultDialog();
                return pingResultDialog.open(validateCustomerDetailsResponse.data.result.isValid, validateCustomerDetailsResponse.data.result.isValid);
            });
    }
}
