...
#pragma warning restore CS0618
        {
            ...
            public async Task<PaymentInfo> AuthorizePaymentAsync(decimal amount, string currency, string voiceAuthorization, bool isManualEntry, ExtensionTransaction extensionTransactionProperties)
            {
                try
                {
                    ...
                    // Get tender
                    TenderInfo maskedTenderInfo = await this.FetchTenderInfoAsync().ConfigureAwait(false);
                    ...
                    if (paymentInfo.IsApproved)
                    {
                        ...
                        {
                            paymentInfo.SignatureData = await this.RequestTenderApprovalAsync(paymentInfo.ApprovedAmount).ConfigureAwait(false);
                        }
                    }
            ...
            {
                await Task.Delay(PaymentDeviceSample.TaskDelayInMilliSeconds).ConfigureAwait(false);
            }
            ...
            public async Task<PaymentInfo> CapturePaymentAsync(decimal amount, string currency, PSDK.PaymentProperty[] paymentProperties, ExtensionTransaction extensionTransactionProperties)
            {
                try
                {
                    var info = new JObject();
            ...
            private async Task<TenderInfo> FillTenderInfo()
            {
                ...
                PaymentCardData cardSwipeData = await getCardTrackData.Task.ConfigureAwait(false);
            ...
            {
                //var signatureInfoTask = Task<string>.Factory.StartNew(async () =>
                //{

                //});

                //return await signatureInfoTask.ConfigureAwait(false);
                ...
                string signature = await signatureData.Task.ConfigureAwait(false);
